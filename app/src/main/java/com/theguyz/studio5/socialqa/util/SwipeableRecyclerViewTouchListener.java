package com.theguyz.studio5.socialqa.util;
...
public class SwipeableRecyclerViewTouchListener implements RecyclerView.OnItemTouchListener {
    private static final String TAG = SwipeableRecyclerViewTouchListener.class.getName();
    ...
    private float mRightViewAlpha;
    private View mLeftView;
    private float mLeftViewAlpha;

    private Button mRightButtonView;
    private float mRightButtonViewAlpha;
    private Button mLeftButtonView;
    private float mLeftButtonViewAlpha;
    ...
    private int[] getRGBFromColor(int color) {
        int red = (color >> 16) & 0xFF;
        int green = (color >> 8) & 0xFF;
        int blue = (color >> 0) & 0xFF;
        float alpha = (color >> 24) & 0xFF;
        int[] returnValue = {red,green,blue};
        return returnValue;
    }
        ...
        switch (motionEvent.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mDownView != null && mAnimatingPosition != mRecyclerView.getChildPosition(mDownView)) {
                    ...
                    QuestionsAdapter.QuestionsViewHolder viewHolder = (QuestionsAdapter.QuestionsViewHolder)mDownView.getTag();
                    mRightView = viewHolder.rightImageView;
                    mLeftView = viewHolder.leftImageView;
                    mRightViewAlpha = mRightView.getAlpha();
                    mLeftViewAlpha = mLeftView.getAlpha();
                    mRightButtonView = viewHolder.rightButton;
                    mLeftButtonView = viewHolder.leftButton;
                    mRightButtonViewAlpha = mRightButtonView.getAlpha();
                    mLeftButtonViewAlpha = mLeftButtonView.getAlpha();
                }
                break;
            }
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (mDownView != null && mSwiping) {
                    ...
                    mRightView.setAlpha(mRightViewAlpha);
                    mLeftView.setAlpha(mLeftViewAlpha);
                    mRightButtonView.setAlpha(mRightButtonViewAlpha);
                    mLeftButtonView.setAlpha(mLeftButtonViewAlpha);
                }
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (dismiss && mDownPosition != mAnimatingPosition && mDownPosition != ListView.INVALID_POSITION) {
                    ...
                    if(dismissRight) {
                        mRightView.setAlpha(mRightViewAlpha);
                        mLeftView.setAlpha(0);
                        mRightButtonView.setAlpha(mRightButtonViewAlpha);
                        mLeftButtonView.setAlpha(0);
                    } else {
                        mRightView.setAlpha(0);
                        mLeftView.setAlpha(mLeftViewAlpha);
                        mRightButtonView.setAlpha(0);
                        mLeftButtonView.setAlpha(mLeftButtonViewAlpha);
                    }
                    ...
                    // cancel
                    Log.i(TAG, "cancelled");
                    mSwipeListener.onSwipeCancelled();
                    mDownView.animate()
                            .translationX(0)
                                    //.alpha(mAlpha)
                            .setDuration(mAnimationTime)
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mSwiping) {
                    ...
                    mSwipeListener.onSwipingEvent(deltaX,mSwipingSlop);

                    if(deltaX > 0) {
                        Log.i(TAG, "Swiping right");
                        mRightView.setAlpha(mRightViewAlpha);
                        mLeftView.setAlpha(Math.max(0f, Math.min(mLeftViewAlpha,
                                mLeftViewAlpha * (1f - Math.abs(deltaX) / mViewWidth))) * 0.75f);
                        mRightButtonView.setAlpha(mRightButtonViewAlpha);
                        mLeftButtonView.setAlpha(Math.max(0f, Math.min(mLeftButtonViewAlpha,
                                mLeftButtonViewAlpha * (1f - Math.abs(deltaX) / mViewWidth))) * 0.75f);

                    } else {
                        Log.i(TAG, "Swiping left");
                        mRightView.setAlpha(Math.max(0f, Math.min(mRightViewAlpha,
                                mRightViewAlpha * (1f - Math.abs(deltaX) / mViewWidth)))*0.75f);
                        mLeftView.setAlpha(mLeftViewAlpha);
                        mRightButtonView.setAlpha(Math.max(0f, Math.min(mRightButtonViewAlpha,
                                mRightButtonViewAlpha * (1f - Math.abs(deltaX) / mViewWidth))) * 0.75f);
                        mLeftButtonView.setAlpha(mLeftButtonViewAlpha);
                    }
    ...
    public interface SwipeListener {
        ...
        void onSwipingEvent(float deltaX, int swipingSlop);
